...
#client = TestClient(app, base_url="https://sd286m9ufnav2uhs5jgng.apigateway-cn-beijing.volceapi.com/")
client = TestClient(app)
timer = PerfTimer()
timer.enable()
def test_slow_layout_api_with_bill_image():
    # 获取测试图片路径
    timer.start_timer("prepare_image")
    ...
    timer.stop_timer("prepare_image")

    # 发送请求（使用默认测试API密钥）
    timer.start_timer("remote_call")
...
def test_fast_layout_api_with_bill_image():
    # 获取测试图片路径
    timer.start_timer("prepare_image")
    test_image_path = os.path.join(os.path.dirname(__file__), 'images', 'bill.png')
    assert os.path.exists(test_image_path), f"测试图片不存在: {test_image_path}"
...
if __name__ == '__main__':
    test_slow_layout_api_with_bill_image()
    test_fast_layout_api_with_bill_image()
